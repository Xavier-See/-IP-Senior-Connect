import RPi.GPIO as GPIO
import time
import json
import paho.mqtt.client as mqtt
from datetime import datetime

# ----- GPIO -----
PROX_PIN = 23  # Change this to the GPIO pin used for Bedroom

# ----- MQTT -----
BROKER_ADDRESS = "raspberrypi.local"   # MUST match controller broker
TOPIC = "senior_connect/sensors/proximity"

# ----- SENSOR INFO (MUST MATCH CONTROLLER) -----
SENSOR_TYPE = "Proximity"
LOCATION = "Bedroom Door"

# ----- GPIO SETUP -----
GPIO.setmode(GPIO.BCM)
GPIO.setup(PROX_PIN, GPIO.IN)

# ----- MQTT CLIENT (v2 API) -----
client = mqtt.Client()

def timestamp():
    return datetime.now().strftime("%H:%M:%S")

def connect_mqtt():
    try:
        client.connect(BROKER_ADDRESS, 1883, 60)
        client.loop_start()
        print("🟢 Connected to Central Controller")
        return True
    except Exception as e:
        print(f"🔴 MQTT Error: {e}")
        return False

def publish(value, status):
    payload = {
        "type": SENSOR_TYPE,
        "location": LOCATION,
        "value": value,
        "status": status
    }
    client.publish(TOPIC, json.dumps(payload))
    print(f"📤 Sent → {payload}")

# ----- MAIN -----
print("🚪 Bedroom Door Proximity Sensor Running...")
print("Press CTRL+C to stop.")

connected = connect_mqtt()
last_state = None   # Track state to prevent duplicate ENTER/EXIT logs

try:
    while True:
        prox_value = GPIO.input(PROX_PIN)

        if prox_value == 1:
            # Sensor detects something (ENTER)
            if last_state != "Detected":
                print(f"[{timestamp()}] 🚪 Bedroom Door Triggered")
                if connected:
                    publish("Detected", "Active")
                last_state = "Detected"

        else:
            # Sensor clear (EXIT)
            if last_state != "Clear":
                print(f"[{timestamp()}] 🚪 Bedroom Door Clear")
                if connected:
                    publish("Clear", "Inactive")
                last_state = "Clear"

        time.sleep(0.2)

except KeyboardInterrupt:
    print("\n🛑 Stopping Bedroom Proximity Sensor...")

finally:
    GPIO.cleanup()
    try:
        client.loop_stop()
        client.disconnect()
    except:
        pass
