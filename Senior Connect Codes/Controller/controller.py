...
# ============================
BEDROOM_VITALS_HR_LOW = 45
BEDROOM_VITALS_BR_LOW = 0
            ...
            if is_bedroom_stream or (hr is not None) or (br is not None):
                ...
                if bed_state == "In Bed" and (hr_low or br_low):
                    ...
                    if now - last_sent >= BEDROOM_VITALS_ALERT_COOLDOWN:
                        ...
                        log_parts = []
                        if hr_low: log_parts.append(f"HR low ({hr_val} < {BEDROOM_VITALS_HR_LOW})")
                        if br_low: log_parts.append(f"BR low ({br_val} < {BEDROOM_VITALS_BR_LOW})")
                        log_to_excel("System", "Bedroom", "CRITICAL", " | ".join(log_parts))
                        bd["last_vitals_alert"] = now
                        print(f"{RED}🚨 BEDROOM VITALS ALERT SENT{RESET}")
                    else:
